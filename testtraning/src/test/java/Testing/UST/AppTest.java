...
public class AppTest {
	...
	public Object[][] dpGetWithParam() {
		Object[][] testDatas = new Object[][] {
				new Object[] { 1, 1, "tom", "jerry", "www.google.com", "www.ust.com", "www.pst.com", 11, "testing", 12,
						"resting", "available" },
				new Object[] { 2, 11, "tomy", "jerry", "www.google.com", "www.ust.com", "www.pst.com", 13, "testing",
						15, "resting", "available" } };
		return testDatas;
	}

	@Test(dataProvider = "dpGetWithParam")
	public void createPetWithData(int petId, int cId, String cName, String name, String p1, String p2, String p3,
			int tId, String tName, int tId_one, String tNameOne, String status) {
...
//				res.then().log().headers();
	...
	public void queryTest1() {
		Response res = RestAssured.given().log().all().baseUri("https://reqres.in/api").queryParam("page", 2)
				.queryParam("id", 5).get("/users");
	...
	public void queryTest() {
		Response res = RestAssured.given().log().all().queryParam("username", "amangarg")
				.queryParam("password", "test1234").get("/user/login");
		res.then().log().all();
		Assert.assertEquals(res.getStatusCode(), 200);
...
//		Assert.assertEquals(re.getMessage(), "20");
	...
	// sign up and provide your info and don't link card and hit the url again on
	// providing details.
	...
	public void doingPaymentWithOauth2Autentication() {
		...
		Response res = RestAssured.given().baseUri("https://api.sandbox.paypal.com/v1").contentType(ContentType.JSON)
				.log().all().auth().oauth2(accessToken).body(file).post("/payments/payment");
	...
	public void uploadTest() {
		...
		Response res = RestAssured.given().log().all().formParam("additionalMetadata", "testing")
				.multiPart(testUploadFile).post(endPoint);
		Assert.assertEquals(res.getStatusCode(), 200);
		res.then().statusCode(200);
		res.prettyPrint();
	}

	// You need to convert your response to bytearray[] from file in case of
	// download.
	// No Other change in download request .. simply hit the endpoint and convert
	// your response to bytearray.
